...
const addBookToDOM = (book) => {
  ...
  pages.textContent = `${book.numberOfPages} page(s)`;
  ...
  // styles already implemented using bootstrap in html
...
const fetchData = (url) => {
  ...
  fetch(url)
    ...
    .then((data) => {
      console.log(data);
      data.forEach((book) => {
        console.log(book.name, book.authors[0], book.numberOfPages, book.isbn);
        addBookToDOM(book);
      });
    })
    .catch((error) => {
